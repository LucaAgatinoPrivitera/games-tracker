		...
		const fetchOwnedGames = async () => {
			try {
				...
				const response = await fetch(
					`/api/IPlayerService/GetOwnedGames/v0001/?key=${apiKey}&steamid=${steamId}&format=json&include_appinfo=true`
				);
					...
					<div key={game.appid} className="flex flex-col p-4 rounded-lg">
						...
						<div className="group relative">
							...
							<div className="opacity-0 absolute inset-0 flex items-center justify-center group-hover:opacity-100 transition-opacity duration-300 bgIcons">
								...
								<a
									onClick={() =>
										handleGameClick(game.appid, game.name, currentPage)
									}
								...
								<a
									onClick={(e) => {
										e.stopPropagation();
										navigate(
											`/achievements/${game.appid}/${encodeURIComponent(
												game.name
											)}?page=${currentPage}`
										);
									}}
